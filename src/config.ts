...
export class Config {
  debugLogging = 'debug';
  'server.port' = '8081';
  'jhipster.clientApp.name' = 'shuttlenet';
  ...
  'jhipster.mail.from' = 'shuttlenet@localhost';
  'jhipster.swagger.default-include-pattern' = '/api/.*';
  'jhipster.swagger.title' = 'shuttlenet API';
  'jhipster.swagger.description' = 'shuttlenet API documentation';
  ...
  'eureka.instance.appname' = 'shuttlenet';
  'eureka.instance.instanceId' = 'shuttlenet:${random.value}';
...
const yamlConfigPath = path.join(__dirname,'..', 'src/config', 'application.yml');
const envYamlConfigPath = path.join(__dirname, '..', 'src/config', `application-dev.yml`);

const yamlConfig = yaml.read(yamlConfigPath);
...
const envYamlConfig = yaml.read(envYamlConfigPath);
